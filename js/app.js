...
const App = {
  ...
  async login() {
    try {
      ...
      this.elements.loginBtn.textContent = 'Iniciando...';

      const deviceFlow = await GitHub.startDeviceFlow();

      // Show device code
      this.elements.deviceCodeDisplay.classList.remove('hidden');
      this.elements.deviceCodeDisplay.querySelector('.device-code').textContent = deviceFlow.user_code;

      // Poll for token
      await GitHub.pollForToken(deviceFlow.device_code, deviceFlow.interval);
      ...
      this.toast(e.message || 'Erro ao fazer login', 'error');
    } finally {
      ...
      this.elements.loginBtn.innerHTML = `
        ...
        Login com GitHub
      `;
      this.elements.deviceCodeDisplay.classList.add('hidden');
    }
  },
